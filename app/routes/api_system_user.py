...
from app.stdio import time_now, print_error, print_success, print_warning
...
from app.core.models import Log, System_Users, System_User_Type

router = APIRouter(prefix="/api/systems_user", tags=["API_System_User"])
...
async def path_systems_user_get_datatable(
    ...
    sql = sql.join(
        System_User_Type, (System_Users.system_user_type_id == System_User_Type.id)
    )
...
async def path_systems_user_get(
    ...
    if id:
        _sql = _sql.where(System_Users.id == id)
        # _sql = _sql.join(System_User_Type, (System_Users.system_user_type_id == System_User_Type.id))
        _system_user: System_Users = (await db.execute(_sql)).mappings().one_or_none()
        print(_system_user)
        return _system_user
    else:
        data: System_Users = (await db.execute(_sql)).mappings().all()
...
async def path_systems_user_me_get(
    ...
    me: System_Users = await get_user_by_id(db, user_jwt)
...
@router.post("/me_change_password")
async def path_systems_user_me_change_pass_post(
    ...
    user: System_Users = await get_user_by_id(db, user_jwt)
    print(user.password)
    print(old_password)
...
async def path_systems_user_me_post(
    ...
    if username != user.username:
        _sql = select(System_Users.id).where(System_Users.username == username)
        ...
        if user.username in ["ROOT", "root"]:
            return {
                "success": False,
                "msg": "root (user already to system state can be changed)",
            }
...
async def path_systems_user_post(
    ...
    print(user)
    if not (
        await allowed_permissions(
            db, user.system_user_type_id, "management_system_user"
        )
    ):
    ...
    if id:
        ...
        _sql = select(System_Users).where(System_Users.id == id)
        _row = (await db.execute(_sql)).one_or_none()
        if not _row:
            return {"success": False, "msg": "item is not already in database"}

        _system_user: System_Users = _row[0]
        if username.lower() == "root" and user != _system_user:
            return {
                "success": False,
                "msg": f"root not already in use please other name !?@#@$)(**&*&^%&(()^&%^$#$#$$^^&^*^",
            }

        if (
            user.username == "root"
            and username != "root"
            and _system_user.username == "root"
        ):
            return {
                "success": False,
                "msg": f"root not chang other name !?@#@$)(**&*&^%&(()^&%^$#$#$$^^&^*^",
            }
        ...
        _sql = select(System_Users).where(System_Users.username == username)
        _system_user = (await db.execute(_sql)).one_or_none()
        if _system_user:
            return {"success": False, "msg": "item is already in database"}

        _system_user = System_Users(
            username=username,
...
async def path_systems_user_delete(
    ...
    if id:
        try:
            _sql = select(System_Users).where(System_Users.id == id)
            # _sql = _sql.join(System_User_Type, (System_Users.system_user_type_id == System_User_Type.id))
            _row: System_Users = (await db.execute(_sql)).one_or_none()
            if _row:
            ...
            if "foreign key constraint" in err:
                return {
                    "success": False,
                    "msg": "ข้อมูลนี้มีการเชื่อมโยงกับข้อมูลอื่นๆ ไม่สามารถดำเนินการได้",
                }
            return {"success": False, "msg": str(e)}
    return {"success": False, "msg": f"Data is not available"}


@router.get("/system_user_type")
async def router_system_user_type(
    user_jwt=Depends(get_jwt_access),
    db: AsyncSession = Depends(get_async_session),
):
    sql = select(System_User_Type)
    result = (await db.execute(sql)).mappings().all()
    return result
...
async def router_api_test(
    ...
    try:
        ...
        await db.commit()
        # db.refresh(log)
        sql = select(Log, System_Users).join(Log, Log.log_owner == System_Users.id)
...
@router.get("/log/{id}")
async def router_api_test_get(
    id: int, db: AsyncSession = Depends(get_async_session), user=Depends(get_jwt_access)
):
    try:
        _query = select(Log, System_Users).join(Log, Log.log_owner == System_Users.id)
